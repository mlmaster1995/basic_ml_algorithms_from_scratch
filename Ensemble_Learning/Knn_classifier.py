...
class KNN:
    ...
    def classify(self, single_dataset, K, recursion=False, showtime=False):
        ...
        if not recursion:
            ...
            distance_tuple_lst = [(self.__standardize_distance[index], self.knn_dataset_target[index]) for index in range(len(self.knn_dataset_target))]
            sorted_distance_tuple_lst = sorted(distance_tuple_lst, key=lambda x: x[0])

            # count class number
            if K <= (len(self.knn_dataset)-1) and K>0:
                target_count = {}
            ...
            # calculate the distance between input data and sample datas
    ...
    def score(self, data_test, data_test_target, K, recursion=False, parallel=True):
        if parallel:
    ...
    def optimize_K(self, data_test, data_test_target, showtime=False):
        ...
        pool = mp.Pool(mp.cpu_count())
        score_list = pool.starmap_async(self.score, [(data_test, data_test_target, i, False, False) for i in list(range(1, max_K))]).get()
        pool.close()
