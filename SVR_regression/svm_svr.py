...
class Svm_Kernel:
    ...
    # train SVM model
    def train(self, C=1, sigma=0.5, degree=2, bias=0, stp=1, epoch_limit=1000, stp_limit=1e-300, stp_show=True, plot=False):
        epoch = 0
        hinge_loss_all = 0
        reg_val = 1 / C
        self.sigma = sigma
        ...
        while epoch <= epoch_limit:
            ...
            for index in range(len(features)):
                ...
                if r >= 1:
                    ...
                    self.weight = self.weight + stp * (self.dataset_target[index] * np.array(features[index]) - reg_val * self.weight)
        ...
        while epoch <= epoch_limit:
            ...
            for index in range(len(self.dataset)):
                ...
                if r >= 1:
                    ...
                    hinge_loss.append(1 - r)
                    self.weight = self.weight + stp * (self.dataset_target[index] * np.array(self.dataset[index]) - reg_val * self.weight)
                hinge_loss_all = sum(hinge_loss)
...
class SVR:
    def __init__(self, dataset, dataset_target):
        # dataet parameters
        self.dataset = np.array(deepcopy(dataset))
        self.dataset_target = np.array(deepcopy(dataset_target))
        self.kernel = 'linear'
    ...
    # train SVM model
    def train(self, C=1, stp=1, epoch_limit=1000, margin_limit=0.01,
              stp_limit=1e-100, stp_show=True, plot=False):
        epoch = 0
        hinge_loss_all = 0
        reg_val = 1 / C
        epoch_plt = []
        ...
        while epoch <= epoch_limit:
            ...
            for index in range(len(self.dataset)):
                r = abs(np.dot(self.dataset[index], self.weight) - self.dataset_target[index])
